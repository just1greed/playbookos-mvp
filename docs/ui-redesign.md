...
- 默认页面
- 是否显示系统导航组
- 环境标签（`environment_label`）

这一页现在回答的是：

- 这个控制台默认应该怎么打开？
- 当前浏览器无本地覆盖时，系统默认行为是什么？
- 这是本地、测试、预发还是生产环境？
...
本轮继续把设置页从“参数编辑”推进到“环境切换”：

- `模型设置` 已新增命名环境能力：
  - 保存当前模型配置为命名环境
  - 从已保存环境中切换当前生效配置
  - 在页面中展示当前 active environment
- `模型设置` 已新增“最近成功配置”持久化：
  - 最近一次成功 probe 的 `tested_at / model / base_url / response_id / output_preview` 会被保存
  - 刷新页面后仍可回看，不再只是本次前端内存态
- `全局设置` 已新增 `environment_label`，用于标记当前控制台所处环境（如 local / staging / prod）
...
- 模型设置里的 profile 删除、配置比较、以及更完整的多环境导入/导出
